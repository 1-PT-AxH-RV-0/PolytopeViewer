...
| 参数名                      | 类型              | 说明                        | 约束条件                                                                              |
|-----------------------------|-------------------|-----------------------------|---------------------------------------------------------------------------------------|
| initialRot                  | number[6]         | 初始旋转角度                | 必须为包含 6 个实数的数组，索引 2、4、5 上的值在非 4D 模式下必须为 0                  |
| initialOfs                  | number[4]         | 初始偏移量（4D）            | 必须为包含 4 个实数的数组，只在 4D 模式下可用                                         |
| initialOfs3                 | number[3]         | 初始偏移量（3D）            | 必须为包含 3 个实数的数组                                                             |
...
2. 字段适用性:
  - `start/end & interp` 适用于: rot、trans4、trans3、setVerticesEdgesDim、setProjDist、setSeparationDist、setFaceScale、setFaceOpacity、setScaleFactor、setCameraLookAt、setCameraDistance 和 setCameraRotation；
...
5. 额外提醒：
  ...
  - 3D 和 4D 的旋转是分顺序的，即先绕 xz 平面旋转再绕 yw 平面旋转和先绕 yw 平面旋转再绕 xz 平面旋转相同的角度，效果（可能）是不一样的。此代码中的旋转顺序由 priority 字段决定。
