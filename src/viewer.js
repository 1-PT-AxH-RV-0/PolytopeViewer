...
const wireframeVisibleSwitcher = document.getElementById(
  'wireframeVisibleSwitcher'
);
const verticesVisibleSwitcher = document.getElementById(
...
const wireframeAndVerticesDimSlider = document.getElementById(
  'wireframeAndVerticesDimSlider'
);
const projectionDistanceSlider = document.getElementById(
  'projectionDistanceSlider'
);
const fileInput = document.getElementById('fileInput');

const rotationSliders = ['XY', 'XZ', 'XW', 'YZ', 'YW', 'ZW'].map(i =>
...
// 创建边和顶点
function createWireframeAndVertices(
  edges,
  {
    cylinderRadius = 1,
...
function create4DWireframeAndVertices(
  edges,
  {
    cylinderRadius = 1,
    sphereRadiusMultiplier = 2,
  ...
  edges.forEach(([start, end]) => {
    ...
    const geometry = toBufferGeometry(
      new THREE.CylinderGeometry(cylinderRadius, cylinderRadius, 1, 5)
    );
...
function loadMesh(meshData, material) {
  ...
  const { wireframeGroup, verticesGroup } = createWireframeAndVertices(
    meshData.edges,
...
function load4DMesh(meshData, material) {
  ...
  const mesh = new THREE.Mesh(geometry, material);
  mesh.material.side = THREE.DoubleSide;
  projectionDistanceSlider.value =
  ...
  const { wireframeGroup, verticesGroup } = create4DWireframeAndVertices(
    meshData.edges,
    {
      cylinderRadius: 0.5 / scaleFactor
    }
...
function loadMeshFromOffData(data, material) {
  ...
  ({ scaleFactor, solidGroup, facesGroup, wireframeGroup, verticesGroup } =
    loadMesh(processedMesh, material));
  updateProperties();
}
...
function loadMeshFrom4OffData(data, material) {
  ...
  ({ scaleFactor, solidGroup, facesGroup, wireframeGroup, verticesGroup } =
    load4DMesh(processedMesh, material));
  updateProperties();
  updateProjectionDistance();
  updateRotation();
}
...
function updateProperties() {
  ...
  changeMaterialProperty(
    wireframeGroup,
    'visible',
    wireframeVisibleSwitcher.checked
  );
  changeMaterialProperty(
    verticesGroup,
    'visible',
    verticesVisibleSwitcher.checked
  );
  ...
  changeCylindersRadius(
    wireframeGroup,
    +wireframeAndVerticesDimSlider.value / scaleFactor
  );
  changeSpheresRadius(
    verticesGroup,
    (+wireframeAndVerticesDimSlider.value / scaleFactor) * 2
  );
}
faceVisibleSwitcher.addEventListener('change', () =>
  changeMaterialProperty(facesGroup, 'visible', faceVisibleSwitcher.checked)
);
wireframeVisibleSwitcher.addEventListener('change', () =>
  changeMaterialProperty(
    wireframeGroup,
    'visible',
    wireframeVisibleSwitcher.checked
...
verticesVisibleSwitcher.addEventListener('change', () =>
  changeMaterialProperty(
    verticesGroup,
    'visible',
    verticesVisibleSwitcher.checked
...
axisVisibleSwitcher.addEventListener('change', () =>
  changeMaterialProperty(axesGroup, 'visible', axisVisibleSwitcher.checked)
);
facesOpacitySlider.addEventListener('input', () =>
  changeMaterialProperty(facesGroup, 'opacity', +facesOpacitySlider.value)
);
  ...
  slider.addEventListener('input', () => {
    ...
    if (!is4D) {
      if (i === 3) solidGroup.rotation.x = +slider.value * (Math.PI / -180);
        ...
        solidGroup.rotation.z = +slider.value * (Math.PI / -180);
    }
  });
});
